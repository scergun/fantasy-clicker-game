document.addEventListener('DOMContentLoaded', function () {
    ...
    let enemyCounter = 0;
    let bossCounter = 0;
    function bossFunc() { bossCounter = (enemyCounter / 5) - 1; };
    ...
    function enemyHandler() {
        function enemyImg() {
            ...
            if (player.currentLevel % 5 === 0 && enemy.level != 1) {
                enemyPortraitImgDOM.src = `${enemyBossArr[(player.currentLevel / 5) - 1].img}`;
                monsterNameDOM.innerHTML = enemyBossArr[(player.currentLevel / 5) - 1].title;
            } else {

                if (enemyCycle === 29) { enemyCycle = 0 };
                enemyPortraitImgDOM.src = `${enemyArr[player.world - 1][enemyCycle].img}`;
    ...
    function tiltAnimation() {
        ...
        enemyPortraitImgDOM.style.filter = 'invert(21%) sepia(100%) saturate(7414%) hue-rotate(359deg) brightness(94%) contrast(117%)'
    ...
    function showBoard2() {
        ...
        <img draggable="false" class="hero-img ${i.photo}" src="${i.img}" alt="">
        <div class="hero-data">
            ...
            <button type='button' data-id='${i.id}' class='${i.isActive}   ${player.money < i.upgradeCost ? 'not-enough' : ''} upgrade'>-${nFormatter(i.upgradeCost)} upgrade</button>
            <button type='button' data-id='${i.id}' class='${!i.isActive} ${player.money < i.hireCost ? 'not-enough' : ''} hire'>-${nFormatter(i.hireCost)} hire</button>
        </div>
    ...
    function nFormatter(num) {
        ...
        return (num / format[formatIndex === -1 ? 6 : formatIndex].value).toFixed(1) + format[formatIndex === -1 ? 6 : formatIndex].symbol;
    };
